...
class RoboschoolForwardWalker(SharedMemoryClientEnv):
    ...
    def create_single_player_scene(self):
        return SinglePlayerStadiumScene(
            gravity=9.8,
            timestep=self.control_time_step / self.sim_frame_skip,
    ...
    def step(self, a):
        ...
        ):  # if multiplayer, action first applied to all robots, then global step() called, then step() for all robots with the same actions
            self.apply_action(a)
        ...
        alive = float(
            self.alive_bonus(state[0] + self.initial_z, self.body_rpy[1])
        ...
        extras = {
            ...
            "MaxAbsTorque": np.max(
                np.abs(self.applied_torques if self.applied_torques else 0)
            ),
            "MeanAbsTorque": np.mean(
                np.abs(self.applied_torques if self.applied_torques else 0)
            ),
        }
