...
class RoboschoolMujocoXmlEnv(gym.Env):
    ...
    def reset(self):
        ...
        self.mjcf = self.scene.cpp_world.load_mjcf(
            os.path.join(os.path.dirname(__file__), "mujoco_assets", self.model_xml)
        )
        self.ordered_joints = []
        self.jdict = {}
        self.parts = {}
    ...
    def render(self, mode="human"):
        if mode == "human":
            ...
            self.camera_adjust()
            rgb, _, _, _, _ = self.camera.render(
                False, False, False
            )  # render_depth, render_labeling, print_timing)
            rendered_rgb = np.fromstring(rgb, dtype=np.uint8).reshape(
                (self.VIDEO_H, self.VIDEO_W, 3)
            )
            return rendered_rgb
        else:
            assert 0
    ...
    def HUD(self, s, a, done):
        ...
        if (
            self.done <= 1
        ):  # Only post score on first time done flag is seen, keep this score if user continues to use env
            s = "%04i %07.1f %s" % (
                self.frame,
                self.reward,
